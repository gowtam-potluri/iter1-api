...
const initialCount = db.patissues.count();
...
for (let i = 0; i < 100; i += 1) {
  ...
  const helpreq = statuses[Math.floor(Math.random() * 4)];
  const quantity = Math.ceil(Math.random() * 20);
  const district = `Lorem ipsum dolor sit amet, ${i}`;
  const id = initialCount + i + 1;

  const issue = {
    id, district, created, phone, name, helpreq, quantity,
  };

  db.patissues.insertOne(issue);
}

const count = db.issues.count();
db.counters.update({ _id: 'patissues' }, { $set: { current: count } });
